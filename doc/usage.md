...
- `GET /logout`: ログアウト
- `GET /root/exists`: rootアカウント存在確認（リダイレクト判定用）

**招待・ユーザー管理エンドポイント:**
- `GET /invite/create`: 招待コード作成（ROOT権限者のみ）
- `GET /invite/list`: 作成した招待コード一覧
- `GET /admin/users`: 登録ユーザー一覧（ROOT権限者のみ）
- `DELETE /admin/users/:user_id`: ユーザー削除（ROOT権限者のみ）
...
- セッション管理とログアウト
- レスポンシブなWebUI
- 自動ページリダイレクト（rootアカウント存在状況に基づく）
- 招待コードサポート（招待URL経由の新規ユーザー登録）
- 招待コード作成・管理（ROOT権限者のみ）
- ユーザー管理（ROOT権限者のみ）

**アクセス方法:**

*初回アクセス時（rootアカウント未登録）:*
1. ブラウザで http://localhost:3000 にアクセス
2. 自動的に登録ページ（/register）にリダイレクト
3. 「Googleアカウントで登録」をクリック
4. Google OAuth認証を完了してrootユーザーとして登録
5. ダッシュボードで保護されたコンテンツを表示
...
*招待URL経由の新規ユーザー登録:*
1. 招待URLにアクセス（例: http://localhost:3000/login?register=true&invite=xxx）
2. 「Googleアカウントで登録」をクリック
3. Google OAuth認証を完了して招待ユーザーとして登録
...
- useEffectフックによる自動リダイレクト処理
- APIクライアントによるrootアカウント存在確認
